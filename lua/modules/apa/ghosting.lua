...
function APA.CheckGhost( ent )
	...
	if trap then
		if APA.Settings.AntiPush:GetBool() then APA.Notify(owner, "Impossible de démonter: Prop obstrué! (Voir Console)", NOTIFY_ERROR, 4, 0, {ent:GetModel(),tostring(trap).."("..trap:GetModel()..")"}) end
		return false 
	end
	return IsValid(ent)
end
...
hook.Add("CanProperty", "APA.CanPropertyFix", function( ply, property, ent )
	if( tostring(property) == "collision" and ent.APGhost ) then
		APA.Notify(ply, "Impossible de définir la propriété sur Ghost.", NOTIFY_ERROR, 4, 0)
		return false 
	end
end)

APA.initPlugin('ghosting')